...
│   ├── package.json
│   └── pnpm-workspace.yaml
├── [projectName]-ai/         # Standalone Mastra service
│   ├── src/
│   │   └── mastra/
│   │       ├── agents/
│   │       ├── tools/
...
│   ├── package.json
│   └── .env
└── [projectName]-apple/      # iOS/macOS/watchOS clients
    ├── Packages/
    │   ├── Auth/
    │   ├── Design/
    │   ├── Core/
...
### Key Principles
- **NO apps/ai** in turborepo - AI functionality lives in standalone `[projectName]-ai`
...
### 1. Standalone AI Service (`[projectName]-ai/`)
```typescript
// src/mastra/index.ts
...
### Phase 1: Structure
- [ ] Remove `apps/ai` from turborepo
- [ ] Create `[projectName]-ai/` with Mastra
- [ ] Create `packages/mastra/`
- [ ] Create/update `packages/api/`
...
### Phase 4: API Layer
- [ ] Create domain services in `packages/api`
...
```bash
# Check for legacy apps/ai
ls apps/ | grep -v "ai" || echo "✓ No apps/ai found"

# Verify packages structure
ls packages/ | grep -E "(api|mastra|database|ui)" || echo "✗ Missing core packages"

# Check Mastra imports
grep -r "@mastra/client-js" packages/api && echo "✗ Direct imports found" || echo "✓ No direct imports"

# Verify workspace deps
grep -r "@repo/mastra" packages/api/package.json && echo "✓ Using workspace dep" || echo "✗ Missing workspace dep"
```
...
When in doubt, follow the patterns established in arbor, radar, and webs projects as they represent the most mature implementations.